	...
	void Update () {
        ...
        if(timeCount >= float.Parse(array[i,0]))
        {
            ...
            */
            ...
            //E, round 19
            skinMeshRenderer.SetBlendShapeWeight(38, values[i,1] + (values[i+1,1] - values[i,1]) / ((times[i+1] - times[i]) / (timeCount - times[i])));
            //F, puckered mouth 20
            skinMeshRenderer.SetBlendShapeWeight(37, values[i,2] + (values[i+1,2] - values[i,2]) / ((times[i+1] - times[i]) / (timeCount - times[i])));
